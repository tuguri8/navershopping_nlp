...
# dataArray = [kid,food,sports,life,culture]


for idx, i in enumerate(food):
    result = []
    for j in range(1,101):
        print(str(idx+1) + "/" + str(len(food)) + "번째 카테고리의 " + str(j) + "페이지")
        req = requests.get('https://search.shopping.naver.com/search/category.nhn?pagingIndex='+str(j)+'&pagingSize=40&viewType=list&sort=rel&cat_id=50000'+str(i)+'&frm=NVSHPAG', headers=headers)
